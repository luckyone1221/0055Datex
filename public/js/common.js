...
function eventHandler() {
	...
	$('.typeit-red-js').each(function () {
		...
		let thisTxt = this.getAttribute('data-type-txt') || this.innerHTML; //define how many symbs there are

		let thisLength = this.innerHTML.length; //define speed

		let speed = 50; // 10 symbs will be printed for 10 * speed(thanks cap XD)

		let typeTime = thisLength * speed; //creare new instanse, this its element from each
	...
	if (!isMobile) {
		let wipeAnimation = new TimelineMax().to("#sOpportunities  .swiper-wrapper", 1, {
			x: "-360%",
		...
		new ScrollMagic.Scene({
			...
			triggerHook: "onLeave",
			duration: "330%"
		}).setPin("#sOpportunities").setTween(wipeAnimation) // .addIndicators() // add indicators (requires plugin)
		.addTo(controller);
	}
	...
	let sClientsSlider1 = new Swiper('.sClients-slider-js', _objectSpread(_objectSpread({}, sClientsSlider), {}, {
		speed: 10000
	}));
	let sClientsSlider2 = new Swiper('.sClients2-slider-js', _objectSpread(_objectSpread({}, sClientsSlider), {}, {
		speed: 5000
	})); //
	...
	makeDDGroup(['.sQuestions-dd-group-js']); //modal trade
	...
	let sideBarItemsMiddle = []; //vanilla scrollLik
	...
	document.addEventListener('scroll', function () {
		if (isMobile) return;
		let scrollTop = window.scrollY; //it calc window.scrollY
	...
	function calcHeaderAltHeight() {
		document.documentElement.style.setProperty('--header-alt-h', "".concat(headerAlt.offsetHeight, "px"));
	}

	if (headerAlt) {
		window.addEventListener('resize', calcHeaderAltHeight, {
			passive: true
		});
		window.addEventListener('scroll', calcHeaderAltHeight, {
			passive: true
		});
		calcHeaderAltHeight();
	} //
	...
	if (cModalInput) {
		window.addEventListener('resize', changeCModalInputPh, {
			passive: true
		});
		changeCModalInputPh();
	}
	...
	function openCModal() {
		...
		$('.cModal--js').each(function () {
			if (this === document.querySelector(".cModal--js[id='".concat(modalId, "']"))) {
	...
	function openNextCModal() {
		...
		let targetModal = document.querySelector(".cModal--js[id='".concat(modalId, "']"));
